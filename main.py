...
with httpx.Client(
    ...
    for page in range(1, 11):
        ...
        logging.info(f'Efetuado requisição no endpoint "{response.url}" e o código de retorno foi "{response.status_code}"')
            ...
            if monetary_value <= desired_price:
                ...
                for country_row in country_table_rows:
                    ...
                    if sale_available_until_element:
                        sale_available_until_value = sale_available_until_element.attrs["title"]
                        break
                sale_available_until = country_row.find("span").attrs["title"]
                country_name = country_row.find("td", class_=None).text.strip()
                game_infos = {**game_infos, "county": country_name, "sale": sale_available_until}
                game_list.append(game_infos)
                logging.info(f'COLETADO o game "{game_infos}"')
            else:
                logging.info(f'Jogo "{game_infos}" NÃO coletado devido ao valor de R$ {monetary_value} estar acima do esperado R$ {desired_price}')
    ...
    logging.info(f'Concluída a raspagem com um total de {len(game_list)} jogos retornados')
